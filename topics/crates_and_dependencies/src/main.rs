...
use fastrand;
use std::{cmp::max, collections::HashMap, thread::yield_now, time::Instant};
...
fn _count_pos_neg() -> (usize, usize, usize) {
    let random20 = (0..20)
...
fn _count_numbers() -> (i32, usize) {
    ...
    let (&most_number, &most_freq) = all_numbers.iter()
...
fn _random_element(arg: &[String]) -> String{
...
fn _benching()-> u128{
    ...
    let _ = (0..1_000_000).map(|_|fastrand::i32(0..100)).collect::<Vec<i32>>();
...
fn main() {
    ...
    */
    ...
    let s = ["Adarsh".to_string(),"Romy".to_string(),"Akshay".to_string()];
    println!("Exercise 8: \n\tRandomly chosen out of Adarsh, Romy, Akshay: {}",random_element(&s));
    */
}
